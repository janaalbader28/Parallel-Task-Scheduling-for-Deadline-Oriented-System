...
void allocateTasks(std::list<Task>& tasks, std::list<Member>& members) {  
    ...
    while (!tasks.empty()) {
        ...
        // Start of parallel section for task allocation
#pragma omp parallel num_threads(2) reduction(+: tasksAssignedToday)
        {
                ...
                for (auto it = tasks.begin(); it != tasks.end();) {
                    ...
                    if (areDependenciesMet(task, completedTasks) && member.assignedHours < member.maxHours) {
                        if (member.assignedHours + task.duration <= member.maxHours) {
                            ...
                            // Private copy of assigned hours
                            int localAssignedHours = member.assignedHours + task.duration;

                            
                            if (localAssignedHours <= member.maxHours) {
                                member.tasks[task.name] = task.duration;
                                member.assignedHours = localAssignedHours;
                                completedTasks[task.name] = true;
                                task.assigned = true;

                               // Erase task from list atomically
                                #pragma omp atomic
                                it = tasks.erase(it);
...
// End of parallel section
        ...
        // If no tasks were assigned today, break the loop
